...
class YicesInstaller(SolverInstaller):
    ...
    def __init__(
        ...
        self.extract_path = os.path.join(
            self.base_dir, "yices2-Yices-%s" % self.solver_version
        )
    ...
    def install_yicespy(self):
        ...
        yicespy_download_link = (
            "https://codeload.github.com/pysmt/yicespy/tar.gz/%s"
            % (yicespy_git_version)
        )
    ...
    def compile(self):
        ...
        SolverInstaller.run(
            "bash configure --prefix %s" % self.yices_path, directory=self.extract_path
        )
