...
class PicoSATInstaller(SolverInstaller):
    ...
    def download(self):
        self.complete_version = "%s.%s" % (
            self.solver_version,
            self.pypicosat_minor_version,
        )
        pypi_link = (
            "https://pypi.python.org/pypi/pyPicoSAT/%s/json" % self.complete_version
        )
    ...
    def compile(self):
        picosat_dir = os.path.join(
            self.extract_path, "picosat-%s" % self.solver_version
        )
        SolverInstaller.run(
            "bash configure.sh",
        ...
        SolverInstaller.run(
            "make", directory=picosat_dir, env_variables={"CFLAGS": " -fPIC"}
        )
        SolverInstaller.run_python("setup.py build", directory=self.extract_path)

    def move(self):
        libdir = "lib.%s-%s-%s" % (self.os_name, self.architecture, self.python_version)
        ...
        SolverInstaller.mv(
            os.path.join(self.extract_path, "picosat.py"), self.bindings_dir
        )
