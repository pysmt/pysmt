...
class TestLogic(TestCase):
    def test_get_logic_by_name(self):
        for l in pysmt.logics.LOGICS:
            ...
            self.assertEqual(l_out, l, "Expecting %s, got %s instead" % (l, l_out))
    ...
    def test_get_logic(self):
        for l in pysmt.logics.LOGICS:
            ...
            self.assertEqual(l_out, l, "Expected %s, got %s instead" % (l, l_out))
    ...
    def test_get_logics_min(self):
        tests = [
            ...
            (pysmt.logics.LRA, get_logic(linear=True, real_arithmetic=True)),
            (
                pysmt.logics.QF_ABV,
                get_logic(quantifier_free=True, arrays=True, bit_vectors=True),
            ),
            (
                pysmt.logics.QF_AUFBV,
                get_logic(
            ...
            (
                pysmt.logics.QF_RDL,
                get_logic(
                    quantifier_free=True, real_arithmetic=True, real_difference=True
                ),
            ),
            (
                pysmt.logics.QF_UFIDL,
                get_logic(
                    quantifier_free=True,
                    uninterpreted=True,
                    integer_arithmetic=True,
