...
Example = namedtuple("Example", ["hr", "expr", "is_valid", "is_sat", "logic"])

SExample = namedtuple("SimpleExample", ["expr", "is_valid", "is_sat", "logic"])
...
def get_full_example_formulae(environment=None):
    ...
    with environment:
        ...
        nested_a = Symbol("a_arb_aii", ArrayType(ArrayType(REAL, BV8), ARRAY_INT_INT))
        ...
        result = [
            ...
            Example(
                hr="((x ? 7 : ((p + -1) * 3)) = q)",
                expr=Equals(Ite(x, Int(7), Times(Plus(p, Int(-1)), Int(3))), q),
                is_valid=False,
                is_sat=True,
                logic=pysmt.logics.QF_LIA,
            ),
            ...
            Example(
                hr="((rg(r) = (5.0 + 2.0)) <-> (rg(r) = 7.0))",
                expr=Iff(
                    Equals(Function(rg, [r]), Plus(Real(5), Real(2))),
            ...
            Example(
                hr="(0_8 u< (((bv8 + 1_8) * 5_8) u/ 5_8))",
                expr=BVUGT(
                    BVUDiv(BVMul(BVAdd(bv8, BVOne(8)), BV(5, width=8)), BV(5, width=8)),
                    BVZero(8),
                ),
                is_valid=False,
                is_sat=True,
            ...
            Example(
                hr="((((1_32 + (- 1_32)) << 1_32) >> 1_32) = 1_32)",
                expr=Equals(
                    BVLShr(BVLShl(BVAdd(BVOne(32), BVNeg(BVOne(32))), 1), 1), BVOne(32)
                ),
                is_valid=False,
                is_sat=False,
                logic=pysmt.logics.QF_BV,
            ...
            Example(
                hr="((0_16 s< bv16) & ((bv16 s/ 65535_16) s< 0_16))",
                expr=And(
                    BVSLT(BVZero(16), bv16),
            ...
            Example(
                hr="((0_16 s< bv16) & ((bv16 s% 1_16) s< 0_16))",
                expr=And(
                    BVSLT(BVZero(16), bv16), BVSLT(BVSRem(bv16, BVOne(16)), BVZero(16))
                ),
                is_valid=False,
                is_sat=False,
                logic=pysmt.logics.QF_BV,
            ...
            Example(
                ...
                expr=ForAll(
                    [r, s],
                    Implies(And(GT(r, Real(0)), GT(s, Real(0))), (LT(Minus(r, s), r))),
                ),
                is_valid=True,
                is_sat=True,
            ...
            Example(
                hr="(exists x . ((x <-> (5.0 < s)) & (s < 3.0)))",
                expr=Exists([x], (And(Iff(x, GT(s, Real(5))), LT(s, Real(3))))),
                is_valid=False,
                is_sat=True,
                logic=pysmt.logics.LRA,
            ),
            ...
            Example(
                hr="(((p - 3) = q) -> ((p < ih(r, (q + 3))) | (ih(r, p) <= p)))",
                expr=Implies(
                    Equals(Minus(p, Int(3)), q),
                    Or(
                        GT(Function(ih, (r, Plus(q, Int(3)))), p),
            ...
            Example(
                hr="(((ToReal((p - 3)) = r) & (ToReal(q) = r)) -> ((p < ih(ToReal((p - 3)), (q + 3))) | (ih(r, p) <= p)))",
                expr=Implies(
                    And(Equals(ToReal(Minus(p, Int(3))), r), Equals(ToReal(q), r)),
                    Or(
                        GT(
                            Function(ih, (ToReal(Minus(p, Int(3))), Plus(q, Int(3)))), p
                        ),
                        LE(Function(ih, (r, p)), p),
                    ),
                ),
                ...
                expr=Not(
                    Implies(
                        ...
                        Or(
                            GT(
                                Function(
                                    ih, (ToReal(Minus(p, Int(3))), Plus(q, Int(3)))
                                ),
                                p,
                            ),
                            LE(Function(ih, (r, p)), p),
            ...
            Example(
                hr='((5 < str.len(str1)) & ("mystr" = str1))',
                expr=And(LT(Int(5), StrLength(str1)), Equals(String("mystr"), str1)),
                is_valid=False,
                is_sat=False,
                logic=pysmt.logics.QF_SLIA,
            ),
            Example(
                hr='((5 = str.len(str1)) & (str.++("my", "str") = str1))',
                expr=And(
                    Equals(Int(5), StrLength(str1)),
            ...
            Example(
                hr='(str.replace("mystr", "str", "my") = "mymy")',
                expr=Equals(
                    StrReplace(String("mystr"), String("str"), String("my")),
                    String("mymy"),
                ),
                is_valid=True,
                is_sat=True,
            ...
            Example(
                hr="('Did you know that any string works? #yolo' & '10' & '|#somesolverskeepthe||' & ' ')"
                "",
                expr=And(
            ...
            #
            Example(
                hr="((q = 0) -> (aii[0 := 0] = aii[0 := q]))",
                expr=Implies(
                    Equals(q, Int(0)),
                    Equals(Store(aii, Int(0), Int(0)), Store(aii, Int(0), q)),
                ),
                is_valid=True,
                is_sat=True,
            ...
            Example(
                hr="(aii[0 := 0][0] = 0)",
                expr=Equals(Select(Store(aii, Int(0), Int(0)), Int(0)), Int(0)),
                is_valid=True,
                is_sat=True,
                logic=pysmt.logics.QF_ALIA,
            ),
            Example(
                hr="((Array{Int, Int}(0)[1 := 1] = aii) & (aii[1] = 0))",
                expr=And(
                    Equals(Array(INT, Int(0), {Int(1): Int(1)}), aii),
            ...
            Example(
                hr="((Array{Int, Int}(0)[1 := 3] = aii) & (aii[1] = 3))",
                expr=And(
                    Equals(Array(INT, Int(0), {Int(1): Int(3)}), aii),
            ...
            Example(
                hr="((Array{Real, Int}(10) = ari) & (ari[6/5] = 0))",
                expr=And(
                    Equals(Array(REAL, Int(10)), ari),
            ...
            Example(
                hr="((Array{Real, Int}(0)[1.0 := 10][2.0 := 20][3.0 := 30][4.0 := 40] = ari) & (! ((ari[0.0] = 0) & (ari[1.0] = 10) & (ari[2.0] = 20) & (ari[3.0] = 30) & (ari[4.0] = 40))))",
                expr=And(
                    Equals(
                        Array(
                            REAL,
                    ...
                    Not(
                        And(
                            Equals(Select(ari, Real(0)), Int(0)),
            ...
            Example(
                hr="((Array{Real, Int}(0)[1.0 := 10][2.0 := 20][3.0 := 30][4.0 := 40][5.0 := 50] = ari) & (! ((ari[0.0] = 0) & (ari[1.0] = 10) & (ari[2.0] = 20) & (ari[3.0] = 30) & (ari[4.0] = 40) & (ari[5.0] = 50))))",
                expr=And(
                    Equals(
                        Array(
                            REAL,
            ...
            Example(
                hr="((a_arb_aii = Array{Array{Real, BV{8}}, Array{Int, Int}}(Array{Int, Int}(7))) -> (a_arb_aii[arb][42] = 7))",
                expr=Implies(
                    Equals(nested_a, Array(ArrayType(REAL, BV8), Array(INT, Int(7)))),
            ...
            Example(
                hr="(abb[bv8 := y_][bv8 := z_] = abb[bv8 := z_])",
                expr=Equals(
                    Store(Store(abb, bv8, Symbol("y_", BV8)), bv8, Symbol("z_", BV8)),
            ...
            Example(
                hr="(((1 - 1) * p * 1) = 0)",
                expr=Equals(Times(Minus(Int(1), Int(1)), p, Int(1)), Int(0)),
                is_valid=True,
                is_sat=True,
                logic=pysmt.logics.QF_LIA,
            ),
            # Huge Fractions:
            Example(
                hr="((r * 1606938044258990275541962092341162602522202993782792835301376/7) = -20480000000000000000000000.0)",
                expr=Equals(Times(r, Real(Fraction(2 ** 200, 7))), Real(-(200 ** 11))),
                is_valid=False,
                is_sat=True,
                logic=pysmt.logics.QF_LRA,
            ),
            Example(
                hr="(((r + 5.0 + s) * (s + 2.0 + r)) = 0.0)",
                expr=Equals(Times(Plus(r, Real(5), s), Plus(s, Real(2), r)), Real(0)),
                is_valid=False,
                is_sat=True,
                logic=pysmt.logics.QF_NRA,
            ),
            Example(
                hr="(((p + 5 + q) * (p - (q - 5))) = ((p * p) + (10 * p) + 25 + (-1 * q * q)))",
                expr=Equals(
                    Times(Plus(p, Int(5), q), Minus(p, Minus(q, Int(5)))),
                    Plus(Times(p, p), Times(Int(10), p), Int(25), Times(Int(-1), q, q)),
                ),
                is_valid=True,
                is_sat=True,
                logic=pysmt.logics.QF_NIA,
