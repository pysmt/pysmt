...
def main():
    ...
    if impl != expected_impl:
        print(
            "Wrong platform detected. %s was expected, but this script "
            "is running on %s!" % (expected_impl, impl)
        )
        exit(2)
    if (
        expected_minor != "*" and expected_minor != str(version.minor)
    ) or expected_major != str(version.major):
        print(
            "Wrong version detected. %s.%s was expected, but this script "
            "is running on %s.%s!"
            % (expected_major, expected_minor, version.major, version.minor)
        )
        exit(2)

    print(
        "All OK. The detected python version is %s %s.%s"
