...
if [ "${TRAVIS_PYTHON_VERSION}" == "3.7" ]; then
    cp -a /opt/python/3.7.1/lib/libpython3.7m.so  ${HOME}/virtualenv/python3.7.1/bin/ || ls ${HOME}/virtualenv/python3.7.1/bin/
    cp -a /opt/python/3.7.1/lib/libpython3.7m.so.1.0  ${HOME}/virtualenv/python3.7.1/bin/ || echo "Could not copy python lib"
    # export LIBRARY_PATH="/opt/python/3.7.1/lib:${LIBRARY_PATH}"
    # export CPATH="/opt/python/3.7.1/include/python3.7m:${CPATH}"
fi

if [ "${TRAVIS_PYTHON_VERSION}" == "2.7" ]; then
    cp -a /opt/python/2.7.15/lib/libpython2.7.so.1.0 ${HOME}/virtualenv/python2.7.15/bin/  || ls ${HOME}/virtualenv/python2.7.15/bin
    cp -a /opt/python/2.7.15/lib/libpython2.7.so ${HOME}/virtualenv/python2.7.15/bin/      || echo "Could not copy python lib"
fi
